...
Builds the two-loop agent architecture from scratch using stub tool
implementations. The stubs prove the loop works before real tool code
(introduced in Lecture 6.2) adds complexity.

Usage:
    python agent.py

    Type a request at the prompt. Empty line to quit.
"""
import os
...
# ─────────────────────────────────────────────
# System prompt (from Lecture 5.3)
# ─────────────────────────────────────────────
...
# Real implementations come in Lecture 6.2
# ─────────────────────────────────────────────

def list_files(path):
    return f"(stub) listing {path}"

def read_file(filename):
    return f"(stub) reading {filename}"

def edit_file(path, old_str, new_str):
    return f"(stub) editing {path}"
